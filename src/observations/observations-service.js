const ObservationsService = {
  ...
  getLastObservationById(knex) {
    return knex.raw(
      "SELECT * FROM (SELECT * FROM observations t1 WHERE (t1.individual_id, t1.time_stamp) = ANY(SELECT t2.individual_id, max(t2.time_stamp) FROM observations t2 GROUP BY t2.individual_id)) raptors JOIN individuals ON raptors.individual_id = individuals.individual_local_identifier;"
    );
  },
  getFirstObservationById(knex) {
    //THIS RETURNS THE RECORD WITH THE MIN TIMESTAMP FOR EACH INDIVIDUAL
    return knex.raw(
      "SELECT * FROM (SELECT * FROM observations t1 WHERE (t1.individual_id, t1.time_stamp) = ANY(SELECT t2.individual_id, min(t2.time_stamp) FROM observations t2 GROUP BY t2.individual_id)) raptors JOIN individuals ON raptors.individual_id = individuals.individual_local_identifier;"
    );
  },
};
